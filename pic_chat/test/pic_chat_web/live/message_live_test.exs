defmodule PicChatWeb.MessageLiveTest do
  ...
  describe "Index" do
    ...
    test "saves new message", %{conn: conn} do
      ...
      assert_redirected(new_live, ~p"/messages")

      {:ok, _, html} = live(conn, ~p"/messages")
    ...
    test "updates message in listing", %{conn: conn} do
      ...
      assert_redirect(edit_live, ~p"/messages")
  ...
  describe "Show" do
    ...
    test "updates message within modal", %{conn: conn} do
      ...
      assert_redirected(edit_live, ~p"/messages/#{message}")

      {:ok, _, html} = live(conn, ~p"/messages/#{message}")
      assert html =~ "some updated content"
    end
  end
end
